...
export async function ageVerificationExample() {
    try {
        ...
        console.log(
            `Website ${websiteDid} requests age verification with nonce: ${challengeNonce.substring(
                0,
                12
            )}...`
        )
        ...
        const proof =
            "AGE_VERIFICATION_PROOF_" +
            Buffer.from(`${userDid}:${challengeNonce}`).toString("base64")
        ...
        const presentationData = JSON.parse(
            decodeBase64(ageVerificationPresentation.presentation)
        )
        const issuerData = JSON.parse(
            decodeBase64(ageVerificationPresentation.issuer)
        )

        // Verify the nonce
        const nonceVerified = presentationData.nonce === challengeNonce
        console.log(
            `Nonce verification: ${nonceVerified ? "✅ VALID" : "❌ INVALID"}`
        )

        // Verify the proof (simulated)
        const proofVerified = ageVerificationPresentation.proof.startsWith(
            "AGE_VERIFICATION_PROOF_"
        )
        console.log(
            `Proof verification: ${proofVerified ? "✅ VALID" : "❌ INVALID"}`
        )
        ...
        console.log("\nVerification result:")
        console.log(
            `User ${disclosedAttributes["firstName"]} ${
                disclosedAttributes["lastName"]
            } is over 18: ${isOver18 ? "✅ YES" : "❌ NO"}`
        )
        ...
        if (isOver18 && nonceVerified && proofVerified) {
            ...
            console.log(
                "🔒 Privacy preserved: Exact birth date was not revealed"
            )
        } else {
