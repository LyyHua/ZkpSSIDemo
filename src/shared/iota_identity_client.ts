...
export function generateNonce(): string {
    ...
    const bytes = Buffer.from(Array(32).fill(0).map(() => Math.floor(Math.random() * 256)));
...
export function createDid(tag: string): string {
    ...
    return `did:iota:${tag}:${Buffer.from(Math.random().toString()).toString('hex').substring(0, 16)}`;
}

/**
 * Parses a DID to extract components
 */
export function parseDid(did: string): {method: string, network?: string, identifier: string} {
