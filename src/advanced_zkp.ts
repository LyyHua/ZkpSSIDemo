...
function simulateBbsSignature(credential: any, privateKey: string): string {
    ...
    const mockSignature = Buffer.from(`${dataToSign}:${privateKey}`).toString(
        "base64"
    )
...
export async function advancedZkpDemo() {
    try {
        ...
        const bbsSignature = simulateBbsSignature(
            healthCredential,
            issuerPrivateKey
        )
        ...
        const revealedValues = {
            bloodType:
                zkpPresentation.verifiableCredential.credentialSubject
                    .bloodType,
            vaccinationStatus:
                zkpPresentation.verifiableCredential.credentialSubject
                    .vaccinationStatus,
